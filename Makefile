...
DOCKER_TAG?=jaytwo_aspnetcore_dataprotection
...
build: restore
	dotnet build ./jaytwo.AspNetCore.DataProtection.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.AspNetCore.DataProtection.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.AspNetCore.DataProtection.Tests.trx"
	reportgenerator \
...
pack:
	rm -rf out/packed
	cd ./src/jaytwo.AspNetCore.DataProtection; \
